...
class ProductKeyService {
  ...
  async isKeyValidated() {
    ...
    try {
      ...
      // Para otros errores de conexión, permitir uso temporal por 3 días
      if (lastValidationDate) {
        const threeDaysAgo = new Date();
        threeDaysAgo.setDate(threeDaysAgo.getDate() - 3);
        
        if (new Date(lastValidationDate) > threeDaysAgo) {
          console.log('Usando clave validada previamente (modo offline por 3 días)');
          return true;
        }
      }
