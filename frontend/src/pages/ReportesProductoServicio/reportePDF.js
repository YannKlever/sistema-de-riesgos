// reportePDF.js - Implementación mejorada con cabecera de empresa
...
export const exportarReporteProductosPDF = async (
  datos, 
  columnasConfig, 
  nombreArchivo = 'reporte_factor_productos_servicios',
  opciones = {}
) => {
  // Validaciones
  ...
  try {
    ...
    doc.setProperties({
      title: `Reporte de Productos/Servicios - ${new Date().toLocaleDateString()}`,
      subject: 'Reporte de gestión de riesgos',
      author: opciones.creador || 'Sistema de Reportes',
      keywords: 'riesgo, productos, servicios, reporte',
    ...
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    doc.text('Reporte de Productos y Servicios', 
    ...
    if (datos.length > 0 && doc.lastAutoTable) {
      ...
      if (finalY < doc.internal.pageSize.height - 50) {
        ...
        const estadisticas = [
          ...
          `Promedio riesgo: ${calcularPromedio('riesgoFactorProductosServicios')}`
        ];
...
export const useExportarPDF = (datos, columnasConfig, opciones = {}) => {
  const exportar = async (nombrePersonalizado = 'reporte_factor_productos_servicios') => {
    try {
      return await exportarReporteProductosPDF(
        datos, 
        columnasConfig, 
        nombrePersonalizado,
        opciones
      ...
      console.error('Error en useExportarPDF:', error);
      throw error;
    }
  };
...
export default { exportarReporteProductosPDF, useExportarPDF };