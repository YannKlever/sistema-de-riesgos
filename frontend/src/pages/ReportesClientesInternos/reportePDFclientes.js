...
// Función principal de exportación a PDF para clientes internos
export const exportarReporteClientesPDF = async (
    datos,
    columnasConfig,
    nombreArchivo = 'reporte_clientes_internos',
    opciones = {}
) => {
    ...
    try {
        ...
        doc.setProperties({
            title: `Reporte de Clientes Internos - ${new Date().toLocaleDateString()}`,
            subject: 'Reporte de gestión de riesgos de clientes internos',
            author: opciones.creador || 'Sistema de Reportes',
            keywords: 'riesgo, clientes internos, reporte',
        ...
        for (let grupoIndex = 0; grupoIndex < gruposColumnas.length; grupoIndex++) {
            ...
            doc.setFontSize(16);
            doc.setFont('helvetica', 'bold');
            doc.text('Reporte de Clientes Internos',
        ...
        const estadisticas = [
            `Total de clientes internos: ${datos.length}`,
...
export const useExportarPDFClientes = (datos, columnasConfig, opciones = {}) => {
    const exportar = async (nombrePersonalizado = 'reporte_clientes_internos') => {
        try {
            return await exportarReporteClientesPDF(
                datos,
                columnasConfig,
                nombrePersonalizado,
                opciones
            ...
            console.error('Error en useExportarPDFClientes:', error);
            throw error;
        }
    };
...
export default { exportarReporteClientesPDF, useExportarPDFClientes };