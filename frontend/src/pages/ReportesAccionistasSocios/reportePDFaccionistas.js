...
// Función principal de exportación a PDF para accionistas/socios
export const exportarReporteAccionistasPDF = async (
    datos,
    columnasConfig,
    nombreArchivo = 'reporte_accionistas_socios',
    opciones = {}
) => {
    ...
    try {
        ...
        doc.setProperties({
            title: `Reporte de Accionistas/Socios - ${new Date().toLocaleDateString()}`,
            subject: 'Reporte de gestión de riesgos de accionistas y socios',
            author: opciones.creador || 'Sistema de Reportes',
            keywords: 'riesgo, accionistas, socios, reporte',
        ...
        for (let grupoIndex = 0; grupoIndex < gruposColumnas.length; grupoIndex++) {
            ...
            doc.setFontSize(16);
            doc.setFont('helvetica', 'bold');
            doc.text('Reporte de Accionistas y Socios',
        ...
        const estadisticas = [
            `Total de accionistas/socios: ${datos.length}`,
            ...
            `Promedio de mitigación aplicada: ${calcularPromedio('mitigacion_numerico')}%`,
...
// Hook para exportar a PDF (mantener igual)
export const useExportarPDFAccionistas = (datos, columnasConfig, opciones = {}) => {
    const exportar = async (nombrePersonalizado = 'reporte_accionistas_socios') => {
        try {
            return await exportarReporteAccionistasPDF(
                datos,
                columnasConfig,
                nombrePersonalizado,
                opciones
            ...
            console.error('Error en useExportarPDFAccionistas:', error);
            throw error;
        }
    };
...
export default { exportarReporteAccionistasPDF, useExportarPDFAccionistas };